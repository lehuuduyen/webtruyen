...
async function run({ sitemapUrl, limit = 50, delayMs = 1500, onProgress } = {}) {
  ...
  `);
  const existsBySlug = db.prepare('SELECT id FROM books WHERE slug = ?');
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    if (preSlug && existsBySlug.get(preSlug)) {
  ...
  while (added < targetNew && !stopFlag.stop) {
    ...
    while (i < allUrls.length && added < targetNew && !stopFlag.stop) {
      ...
      try {
        ...
        if (!data.title || !data.slug || existsBySlug.get(data.slug)) {
          skipped++;
        } else {
          insertBook.run({
            id: `${Date.now()}${Math.random().toString(36).slice(2, 6)}`,
