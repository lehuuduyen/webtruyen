    ...
    <nav aria-label="Danh mục thể loại" className="space-y-1">
      ...
      <Link
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-all ${
          pathname === '/the-loai'
            ? 'bg-rose-100 text-pink-600 border border-rose-600/40'
              ...
              <Link
                ...
                className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-all ${
                  isActive(root.slug)
                    ...
                    : isRootActive
                    ? 'text-pink-600'
                    : 'text-gray-700 hover:text-gray-900 hover:bg-rose-50'
                }`}
              >
                ...
                {children.map(child => (
                  <Link
                    ...
                    className={`flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs transition-all ${
                      isActive(child.slug)
                        ? 'bg-rose-100 text-pink-600 font-semibold'
      ...
      <Link
        href="/bang-xep-hang"
        className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-500 hover:text-gray-900 hover:bg-rose-50 transition-all"
      >
        <span className="text-base" aria-hidden="true">🏆</span>
        Bảng xếp hạng
      </Link>
